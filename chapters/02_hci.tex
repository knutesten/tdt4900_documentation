...
\gls{hci} is the science of the interaction between humans and computers, the goal of \gls{hci} is to make this interaction as smooth and seamless as possible. Research done in the \gls{hci}-field have resulted in guidelines and design methods for the creation of efficient and usable software. One important factor when focusing on usability is to include the future users the design process to get feedback on the user interface.

\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  thick,main node/.style={circle,fill=blue!20,draw,font=\sffamily\Large\bfseries}]

  \node[main node] (1) at (0, -2) {prot};
  \node[main node] (2) at (-3, 0) {design};
  \node[main node] (3) at (0, 2) {req};
  \node[main node] (4) at (3, 0) {eval};
...
The figure above shows an interaction design lifecycle model. The model shows an iterative approach to the design, starting with gathering and defining requirements. The requirements are used as basis for designing the user interface, during the design phase. Prototypes are then created to be evaluated by the users. After an evaluation the feedback on the prototype is used to revise the user interface design or the requirements. When the evaluation gives satisfactory results the product completed, often after several iterations.
