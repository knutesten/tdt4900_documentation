...
%Avsinttet var veldig tynt, starter ganske bra, men så plutselig hoppper du til iterasjonsprossesen. Du nevner at en factor er å putte brukeren i sentrum, heter ikke dette user centered design? Hva slags andre metodolgier er det. Dette med design prinsippene (iterasjonene) er veldig fint å ha med men burde kanskje skrive litt mer om selveste feltet først og design prinsippene. Det er greit å ha med siden vi tross alt har et kapittel som heter research design hvor vi skriver hvorfor vi akkurat valgte det vi gjorde. Du mangler kilder på alt, motherfucking sources son!
...
%According to \textit{Experimentation in Software Engineering} \cite{experimentInSE} the valdiity of a study denotes the trustworthiness of results. To what extent these are true and not biased by researches subjective point of view. 

According to \textit{Interaction Design: beyond human-computer interaction} validity is concerned with whether the evaluation method measures what it is intended to measure. This 
applies to both the method itself and the way it is executed. For example if one wishes to examine how a product is used in the home setting up a controlled lab experiment would hurt the validity. Wrong choice of method and execution are not the only threats to validity, bias and the Hawthorne effect threaten validity but can be impossible or hard to avoid.

Ecological Validity also known as the Hawthorne effect is concerned with how the evaluation method and environment themselves influence and affect the results. If a subject aware that the are being observed and the test is conducted outside of their natural surroundings their behaviour and answers will be influenced. Bias is when the results are distorted by the researchers and evaluators. Evaluators might fail to note certain behaviour because they deem it irrelevant, or an interviewers tone of voice may influence the answers of the interviewee. It is therefore important to be open to the possibility of bias.

%Vet ikke om vi skal ha med det under men jeg slengte det på
Validity in HCI is a complex area and we have only mentioned some of the basic issues. Thimbleby reviews some of the more complex issues and suggests some practical recommendations to solving them in his \textit{Validity and Cross-Validity in HCI Publications} paper \cite{validation}. The simplest suggestion he makes is to use Triangulation i.e use multiple paths to achieve the same result. A more complex one involves creating a universal star rating system, where  papers should be rated based on how easily a researcher can reproduce or build upon the original work.
